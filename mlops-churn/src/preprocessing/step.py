...
Implementa PipelineStep para a etapa de feature engineering:
  Entrada : data/processed/telco_customer_churn.parquet  (gerado por ingestao + qualidade)
...
Responsabilidades:
  ...
  3. Aplicar fit_transform (todas as etapas stateless + GroupMedianImputer)
...
class PreprocessingStep(PipelineStep):
    ...
    def _logar_resumo_config(self) -> None:
        ...
        self.logger.info("  Type cast           : %d", len(self._config.get("type_cast", [])))
        self.logger.info("  Flags binárias      : %d", len(self._config.get("binary_flags", [])))
        self.logger.info("  Imputações          : %d", len(self._config.get("imputation", [])))
        self.logger.info(
            "  Encoding binário    : %d",
            len(self._config.get("binary_encoding", {}).get("columns", [])),
        )
        self.logger.info(
            "  Encoding ternário   : %d",
            len(self._config.get("ternary_encoding", {}).get("columns", [])),
        )
        self.logger.info(
            "  Encoding categórico : %d",
            len(self._config.get("categorical_encoding", [])),
        )
