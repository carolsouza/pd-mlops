...
def _params_reducer_padrao(feat_red_cfg: dict) -> dict:
    ...
    if metodo == 'rfe':
        ...
        params['lda_solver'] = cfg_metodo.get('lda_solver', 'svd')
...
class OptunaOptimizer(BaseOptimizer):
    ...
    Objetivo: maximizar ROC-AUC (robusta ao desbalanceamento 73%/27%).
        ...
        def _objetivo(trial: optuna.Trial) -> float:
            ...
            with mlflow.start_run(
                ...
                mlflow.log_metrics({
                    ...
                    'cv_recall_mean' : agg['cv_recall_mean'],
                })
...
class GridSearchOptimizer(BaseOptimizer):
    ...
    cv_runner : CVRunner configurado (usa o StratifiedKFold internamente via n_splits)
    seed      : semente aleatória global
    logger    : Logger opcional
    """
...
class OptimizerFactory:
    ...
    def criar(
        ...
        if strategy == 'optuna':
            ...
            raise ValueError(
                f"Estrategia de otimizacao desconhecida: '{strategy}'. "
                "Valores validos: 'optuna', 'grid_search', 'randomized_search'."
            )
