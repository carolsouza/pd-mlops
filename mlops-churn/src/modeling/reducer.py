...
(seção feature_reduction):
    ...
    lda    → Linear Discriminant Analysis (supervisionada, maximiza separação entre classes)
...
class FeatureReducer(BaseEstimator, TransformerMixin):
    ...
    ----------
    method : str
        Estratégia de redução: 'none' | 'rfe' | 'pca' | 'kpca' | 'lda'.
    ...
    feature_names_out_ : list[str] | None
        ...
        PCA/kPCA usa 'pc_0', 'pc_1', ...; LDA usa 'ld_0', 'ld_1', ...).
    """
    ...
    def _construir_redutor_interno(self):
        ...
        raise ValueError(
            ...
            "Opções válidas: 'none', 'rfe', 'pca', 'kpca', 'lda'."
        )
    ...
    def fit(self, X, y=None) -> "FeatureReducer":
        ...
        if self.method == 'rfe':
            ...
            if y is None:
                raise ValueError(
                    "FeatureReducer com method='lda' requer y. "
                    "Certifique-se de que está dentro de um Pipeline que recebe y."
                )
            self.reducer_.fit(X, y)
            # LDA produz no máximo (n_classes - 1) componentes.
            # Para classificação binária (Churn 0/1) isso resulta em 1 componente.
            n_out = self.reducer_.scalings_.shape[1]
            self.feature_names_out_ = [f'ld_{i}' for i in range(n_out)]
            self._logar(
                "FeatureReducer.fit: LDA ajustado → %d componente(s) discriminante(s) "
                "(máx = n_classes - 1 = %d).",
                n_out, len(np.unique(y)) - 1,
            )
